from database import conn,posts_table,Select
...
@posts_router.get("/post/{passed_id}", status_code=status.HTTP_200_OK)
async def post_by_id(passed_id: int, status_code: Response):
    post = find_post_in_db(passed_id)
    return post
  
    
@posts_router.post("/createpost",status_code=status.HTTP_201_CREATED)
...
@posts_router.delete("/delete/post/{passed_id}")
